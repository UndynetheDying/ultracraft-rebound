...
public abstract class AbstractPedestalBlock extends BlockWithEntity implements IPunchableBlock, BlockEntityProvider
{
	public static final DirectionProperty FACING = HorizontalFacingBlock.FACING;
	public static final Property<Boolean> FANCY = BooleanProperty.of("fancy"), LOCKED = BooleanProperty.of("locked");
	
	public AbstractPedestalBlock(Settings settings)
	{
		super(settings);
	}
	...
	{
		builder.add(FACING, FANCY, LOCKED);
	}
	...
	public void onStateReplaced(BlockState state, World world, BlockPos pos, BlockState newState, boolean moved)
	{
		if(state.getBlock() != newState.getBlock())
		{
			...
			if(entity instanceof AbstractPedestalEntity pedestal && world.getGameRules().getBoolean(GameRules.DO_TILE_DROPS) && !pedestal.decorative)
	...
	public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit)
	{
		...
		if (!state.get(FANCY) && stack.isOf(Items.GLOWSTONE_DUST))
		{
		...
		else if(stack.isOf(Items.WATER_BUCKET) && canBeCleaned(state))
		{
			cleanPedestal(world, pos, state);
			if(!player.isCreative())
		...
		else if(!state.get(LOCKED) && !player.isSneaking())
		{
			...
			{
				BlockEntity blockEntity = world.getBlockEntity(pos);
				if(blockEntity instanceof AbstractPedestalEntity)
	...
	public void scheduledTick(BlockState state, ServerWorld world, BlockPos pos, Random random)
	{
		...
		if(world != null && world.getBlockEntity(pos) instanceof AbstractPedestalEntity pedestal)
		{
			pedestal.sacrifice();
			updateNeighbors(world, pos);
		}
