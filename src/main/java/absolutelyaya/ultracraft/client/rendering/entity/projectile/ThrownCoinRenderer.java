...
{
	final Identifier COIN_TEXTURE = Ultracraft.texIdentifier("textures/entity/coin");
	...
	public void render(ThrownCoinEntity entity, float yaw, float tickDelta, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light)
	{
		...
		if((entity.getVelocity().length() > 0f && entity.isSplittable()) || !entity.isUnused())
		{
			...
			matrices.multiply(RotationAxis.POSITIVE_Z.rotation((entity.age + tickDelta) * entity.getFlashRotSpeed()));
