...
public class TitleBGRenderer extends RotatingCubeMapRenderer
{
	...
	void drawBG(MinecraftClient client, float alpha, float v)
	{
		...
		Matrix4f matrix4f = new Matrix4f().perspective(85f * 0.0174f, (float)client.getWindow().getFramebufferWidth() / (float)client.getWindow().getFramebufferHeight(), 0.05F, 10f);
		...
		{
			matrixStack.push();
			matrixStack.translate(0, (i - 1) * 2 + yOffset, 0);
			RenderSystem.applyModelViewMatrix();
			bufferBuilder.begin(VertexFormat.DrawMode.QUADS, VertexFormats.POSITION_TEXTURE_COLOR);
			bufferBuilder.light(0xffffffff);
			int l = Math.round(255f * alpha);
			float s = 1f / 8f;
			bufferBuilder.vertex(1f, -1f, 1f).texture(0f, 0f + v + (i * s)).color(255, 255, 255, l).next();
			bufferBuilder.vertex(1f, 1f, 1f).texture(0f, s + v + (i * s)).color(255, 255, 255, l).next();
			bufferBuilder.vertex(1f, 1f, -1f).texture(1f, s + v + (i * s)).color(255, 255, 255, l).next();
			bufferBuilder.vertex(1f, -1f, -1f).texture(1f, 0f + v + (i * s)).color(255, 255, 255, l).next();
